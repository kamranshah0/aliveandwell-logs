...
const EditUserModal = ({
  ...
  const { isEditUserModal, selectedUser, closeEditUserModal } = useModalStore();
  ...
  useEffect(() => {
    if (selectedUser) {
      reset({
        ...
        cellNo: selectedUser.cellNo || "",
      });
      if (selectedUser.profileImg) {
        setPreview(selectedUser.profileImg);
      }
    }
  }, [selectedUser, reset]);
        ...
        prev.map((u) =>
          u.id === selectedUser?.id
            ? {
                ...
                profileImg: preview || u.profileImg,
              }
            : u
        )
      );
  ...
  return (
    ...
    >
      <DialogContent className="bg-surface-0 rounded-2xl p-0 md:min-w-[650px] max-h-[90vh] flex flex-col gap-0 overflow-hidden">
        <DialougeHeader title="Edit User" />
